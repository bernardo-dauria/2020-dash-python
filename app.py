    ...
    html.Div([
        ...
        html.Div([
            html.Div(
                dcc.RangeSlider(
                    id='my-slider',
                    min=0, max=100, value=[0, 100], step=0.1,
                ),
                style={
                    'width': '60%',
...
def update_slider(input_value):
    ...
    marks = {
            "min": {'label': str(min), 'style': {'color': '#77b0b1'}},
...
@app.callback(
    [Output('my-table', 'data'), Output('my-table', 'columns')],
...
def display_selected_data(selected_data):
    ...
    return df[df['name'].isin(names)].to_dict("rows"), [{"name": i, "id": i} for i in df.columns]
